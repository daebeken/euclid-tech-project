# [6] TIME SERIES MODEL: time series features from tsfeatures package which mainly utilises on 2 other packages: hctsa and kctsa which contains time series features
# [9] ROLLINGTSFEATURES IS OK
# https://cran.r-project.org/web/packages/tsfeatures/index.html

# @description: creates time series features from various packages such as hctsa and kctsa
# @description: computes using closing price

#======= Main Class =======
# create R6 class
feature_seven <- R6::R6Class(
	"feature_seven",
    private = list(
        load_dependencies = function(){
            required_packages <- c("tidyverse", "checkmate", "duckdb", "data.table", "mirai", "crew", "parallel", "tictoc", "QuantTools", "bidask", "roll", "TTR", "PerformanceAnalytics",
				"binomialtrend", "vse4ts", "exuber", "GAS", "tvgarch", "quarks", "ufRisk", "theftdlc", "theft", "tsfeatures")
	...
	public = list(
        ...
        # @returns: values given by that time series method
            ...
            # not using compengine because it is a repeat of several others
            # compengine is a subset of autocorr_features, pred_features, station_features, dist_features, and scal_features
            # Mr Mislav did not use autocorr_features, pred_features, "scal_features": which means we are left with dist_features and station_features
            
            feature_list <- c("acf_features", "arch_stat", "compengine", "crossing_points", "entropy", "firstzero_ac", "flat_spots",
                "heterogeneity", "dist_features", "holt_parameters", "hurst", "localsimple_taures", "lumpiness", "stability",
                "max_level_shift", "max_kl_shift", "max_var_shift", "nonlinearity", "pacf_features", "sampenc", "station_features",
        ...
        run_code = function(subset_df, windows_, price_col, rolling_function){
            ...
            # chunked_indices <- split(indices, ceiling(seq_along(indices) / batch_size))
            ...
            for (j in seq_along(stat_names)) {
                colname <- paste0("tsfeatures_", stat_names[j])
        ...
        other_finfeatures_indicators = function(symbol_df) {
            ...
            print("=================== [9] COMPUTING time series features from tsfeatures Package ===================")
            #=================== OTHER INDICATORS 9: VaR and Expected Shortfall from tsfeatures Package ===================
            print("[9] Calculating time series features from tsfeatures Package...")
            ...
            print(paste0("Total Time Taken to Calculating time series features from tsfeatures Package ", tsfeatures_time, " Seconds"))
            # return(res_list)s
            rm(start_time, windows_, price_col, try_list, results_dt, new_cols, end_time, tsfeature_time)
...
# feature_loader$rolling_function(tsfeature_dt, window, "close")
...
# # feature_func <- get(method, envir = asNamespace("tsfeatures"))
# # feature_func(tsfeature_dt[, get(price_col)])