# [7] TIME SERIES MODEL: NON LINEAR AUTOREGRESSIVE : implementing a self exciting threshold autoregressive (SETAR) model
# 10: ROLLINGTSDYN IS OK
# https://cran.r-project.org/web/packages/tsDyn/index.html

# @description: Implements nonlinear autoregressive (AR) time series models
# @description: computes using RETURNS (NOT CLOSING PRICE!!!)

#======= Main Class =======
# create R6 class
feature_eight <- R6::R6Class(
	"feature_eight",
    private = list(
        load_dependencies = function(){
            required_packages <- c("tidyverse", "checkmate", "duckdb", "data.table", "mirai", "crew", "parallel", "tictoc", "QuantTools", "bidask", "roll", "TTR", "PerformanceAnalytics",
				"binomialtrend", "vse4ts", "exuber", "GAS", "tvgarch", "quarks", "ufRisk", "theftdlc", "theft", "tsfeatures", "tsDyn", "fracdiff")
	...
	public = list(
        ...
        # @returns: values given by that time series method
            ...
            forecast_type = "setar_"
            m = 2
            nthresh = 1

            y <- tsDyn::setar(na.omit(x$returns), m = m, nthresh = nthresh) # fitting non linear model
            y <- tsDyn:::predict.nlar(y, n.ahead = 5, type = "bootstrap") # prediction
            y <- as.data.table(y$pred) # extracting the prediction

            # Compute outputs
            first_forecasts <- as.numeric(y[1, ])
            last_forecasts <- as.numeric(y[nrow(y), ])
            mean_forecasts <- as.numeric(apply(y, 2, mean, na.rm = TRUE))
            sd_forecasts <- as.numeric(apply(y, 2, sd, na.rm = TRUE))

            # Combine all outputs into a single row
            output <- as.data.table(
                matrix(c(first_forecasts, last_forecasts, mean_forecasts, sd_forecasts), 
                        nrow = 1)
                )

            # Generate proper column names
            forecast_steps <- colnames(y)
            colnames(output) <- gsub(" ", "", c(
                paste0(forecast_type, "1_", window, "_", forecast_steps),
                paste0(forecast_type, "last_", window, "_", forecast_steps),
                paste0(forecast_type, "mean_", window, "_", forecast_steps),
                paste0(forecast_type, "sd_", window, "_", forecast_steps)
            ))
        ...
        run_code = function(subset_df, windows_, price_col, rolling_function){
            ...
            results <- mirai::mirai_map(
                ...
                .f = function(index_chunk, dt, windows_, rolling_function, price_col) {
                    ...
                    out <- lapply(index_chunk, function(i) {
                        ...
                        rolling_function(window_data, windows_)
                })
            ...
            for (j in seq_along(stat_names)) {
                colname <- paste0("tsdyn_", stat_names[j])
        ...
        other_finfeatures_indicators = function(symbol_df) {
            ...
            print("=================== [10] COMPUTING time series features from tsDyn Package ===================")
            #=================== OTHER INDICATORS 10: SETAR model from tsDyn Package ===================
            print("[10] Calculating time series features from tsDyn Package...")
            ...
            end_time <- tictoc::toc()
            tsfeatures_time <- round(end_time$toc - end_time$tic,3)
            print(paste0("Total Time Taken to Calculating time series features from tsDyn Package ", tsfeatures_time, " Seconds"))
            rm(start_time, windows_, price_col, try_list, results_dt, new_cols, end_time, tsfeatures_time)
...
# # # working on tsdyn package
# # tsfeature_dt <- tsfeature_dt[, returns := get("close") / data.table::shift(get("close")) - 1][!is.na(returns)]
# # feature_loader$rolling_function(tsfeature_dt, window)

# # forecast_type = "setar"
# # m = 2
# # nthresh = 1

# # y <- tsDyn::setar(na.omit(tsfeature_dt$close), m = m, nthresh = nthresh) # fitting non linear model
