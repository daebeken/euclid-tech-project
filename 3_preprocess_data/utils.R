# path_params=symbol; api_version = 'v3'; endpoint = endpoint; query_list = query_list
# base_url='cloud'

get.endpoint.content.f <- function(path_params=NULL, endpoint, query_list, base_url = NULL, FMP_API_KEY){
  Sys.setenv("FMP_API_KEY"=FMP_API_KEY)
  request_urls <- build_request_urls(path_params, endpoint = endpoint, query_list = query_list, base_url = base_url)
  rst <- get_request_content(request_urls, endpoint = endpoint)
  return(rst)
}
...
#' @noRd
build_request_urls <- function(path_params, endpoint, query_list = NULL, base_url = NULL) {
  ...
  if (Sys.getenv('FMP_API_KEY') != '')  {
    ...
    warning_message <- glue::glue('An API Key is required to use NicoGPT.')
    
    warning(warning_message)
    apikey <- 'demo'
  }
  
  url_path_stem <- paste0(c('stable', endpoint), collapse = "/") # old: paste0(c('api', api_version, endpoint), collapse = "/")
  if (!is.null(path_params)){
  	path_params <- paste0("symbol=", path_params) # added this line in
  	url_path_stem <- paste(url_path_stem, path_params, sep = "?") # changed "/" to "?"
  	}
  ...
  # if (is.null(base_url)){base_url <- get_base_url('fmp')} else {base_url <- get_base_url('cloud')}
  ...
  endpoint_urls_df$request_url <- glue::glue_data(endpoint_urls_df, '{base_url}/{path}&{query}') # changed "?" to "&"
...
format_request_content <- function(df, arguments) {
  ...
  d <- dplyr::mutate(d, dplyr::across(all_of(char_cols), readr::parse_guess))
  d <- dplyr::mutate(d, dplyr::across(all_of(lgl_cols), as.numeric))
